...
class AddGameViewController: UIViewController, UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    @IBOutlet weak var yourDeckField: UITextField!
    @IBOutlet weak var opponentsDeckField: UITextField!
    ...
    @IBAction func addGameButtonTouchUp(sender: UIButton) {
        let yourDeck = self.heroes[self.heroPicker.selectedRowInComponent(HERO_PICKER)]
        let opponentsDeck = self.heroes[self.heroPicker.selectedRowInComponent(OPPONENTS_HERO_PICKER)]

        let won = self.wonSwitch.on
        let coin = self.coinSwitch.on
        TrackOBot.postResult(yourDeck, opponentsDeck:opponentsDeck, won:won, coin:coin, onComplete:{
            (res, err) -> Void in
            if let error = err {
